...
{% set fao_source = source('bronze', 'fao_prices_raw') %}
...
typed as (
    select
        {{ column_or_null(fao_source, ['commodity', 'item', 'item_name', 'product', 'product_name', 'name']) }} as raw_commodity,
        {{ column_or_null(fao_source, ['date', 'price_date', 'period', 'year_month', 'month_date']) }} as raw_date,
        {{ column_or_null(fao_source, ['year'], 'integer') }} as raw_year,
        {{ column_or_null(fao_source, ['month'], 'integer') }} as raw_month,
        {{ column_or_null(fao_source, ['region', 'market', 'market_name', 'area', 'province']) }} as raw_region,
        {{ column_or_null(fao_source, ['country', 'country_name']) }} as raw_country,
        {{ column_or_null(fao_source, ['price', 'value', 'price_usd', 'price_usd_per_kg'], 'decimal(18,6)') }} as raw_price,
        {{ column_or_null(fao_source, ['currency']) }} as raw_currency,
        {{ column_or_null(fao_source, ['unit', 'price_unit']) }} as raw_unit,
        {{ column_or_null(fao_source, ['ingested_at', 'loaded_at', 'created_at'], 'timestamp') }} as ingested_at
    from raw
),
...
standardized as (
    select
        ...
        coalesce(nullif(trim(typed.raw_region), ''), 'unknown') as region,
        coalesce(nullif(trim(typed.raw_country), ''), 'Vietnam') as country,
        case
